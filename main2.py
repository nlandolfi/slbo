...
from rllab.envs.mujoco.half_cheetah_task_env import HalfCheetahTaskConfig
...
def main():
    assert FLAGS.env.id == "HalfCheetahTask-v2", "only tested for HalfCheetahTask-v2"

    FLAGS.set_seed()
    FLAGS.freeze()

    task = HalfCheetahTaskConfig()
    ...
    # Number of stages is number of tasks sampled.
    for T in range(FLAGS.slbo.n_stages):
        logger.info('------ Starting Stage %d --------', T)
        task.sample()
        logger.info(f'Task Sampled: {task}')
        ...
        if T == 0:
            ...
            for n_updates in range(FLAGS.slbo.n_policy_iters):
                ...
                logger.info('[TRPO WARMUP] # %d: n_episodes = %d, returns: {mean = %.0f, std = %.0f}, '
