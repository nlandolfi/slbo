...
def main():
    ...
    policies = [
        GaussianMLPPolicy(dim_state, dim_action, normalizer=normalizers.state, **FLAGS.policy.as_dict()) for i in range(FLAGS.task.skip_window)
    ]
    # batched noises
    ...
    algos = [TRPO(vfn=vfn, policy=policy, dim_state=dim_state, dim_action=dim_action, **FLAGS.TRPO.as_dict()) for policy in policies]
    ...
    # TODO/HACK/NOTE: this will not save the policy correctly but we don't care
    saver = nn.ModuleDict({'policy': policies[-1], 'model': model, 'vfn': vfn})
    print(saver)
    ...
    while TASK_NUM < FLAGS.task.n_iters:
        ...
        settings = [(runners['test'], policies[pidx], 'Real Env'), (runners['train'], policies[pidx], 'Virt Env')]
        for (i, runner) in enumerate(shadow_runners):
            settings.append((runner, policies[pidx], f'Shadow Env-{i}'))
        ...
        if FLAGS.task.reset_policy:
            ...
            tf.get_default_session().run(tf.variables_initializer(policies[pidx].parameters()))
        ...
        for i in range(FLAGS.warmup.n_iters):
            ...
            for n_updates in range(FLAGS.warmup.n_policy_iters):
                ...
                data, ep_infos = runners['train'].run(policies[pidx], FLAGS.plan.n_trpo_samples)
                advantages, values = runners['train'].compute_advantage(vfn, data)
                dist_mean, dist_std, vf_loss = algos[pidx].train(max_ent_coef, data, advantages, values)
        ...
        if TASK_NUM > 0 or FLAGS.ckpt.model_load: # i.e., we did the warmup
            if FLAGS.task.skip_policy == 'none':
                ...
                for runner in shadow_runners:
                    data, ep_infos = runner.run(policies[pidx], FLAGS.rollout.n_test_samples)
                    returns = [info['return'] for info in ep_infos]
                ...
                data, ep_infos = r0.run(policies[pidx], FLAGS.rollout.n_test_samples)
                ...
                real_data, ep_infos = runners['test'].run(policies[pidx], FLAGS.rollout.n_test_samples)
                real_returns = [info['return'] for info in ep_infos]
                _, ep_infos = runners['train'].run(policies[pidx], FLAGS.rollout.n_test_samples)
        ...
        spidx = np.argmax(skip_metrics[-FLAGS.task.skip_window:])
        logger.info(f"Selected Policy Index {spidx}")
        logger.info(f"Selected Task Is {tasks[-FLAGS.task.skip_window:][spidx]}")

        settings = [(runners['test'], policies[spidx], 'Real Env'), (runners['train'], policies[spidx], 'Virt Env')]
        for (i, runner) in enumerate(shadow_runners):
            settings.append((runner, policies[spidx], f'Shadow Env-{i}'))
        ...
        for T in range(FLAGS.slbo.n_stages):
            ...
            # collect data
            recent_train_set, ep_infos = runners['collect'].run(noise.make(policies[spidx]), FLAGS.rollout.n_train_samples)
            add_multi_step(recent_train_set, train_set)
            add_multi_step(
                runners['dev'].run(noise.make(policies[spidx]), FLAGS.rollout.n_dev_samples)[0],
                dev_set,
            )
            ...
            for i in range(FLAGS.slbo.n_iters):
                ...
                for n_updates in range(FLAGS.slbo.n_policy_iters):
                    ...
                    data, ep_infos = runners['train'].run(policies[spidx], FLAGS.plan.n_trpo_samples)
                    advantages, values = runners['train'].compute_advantage(vfn, data)
                    dist_mean, dist_std, vf_loss = algos[spidx].train(max_ent_coef, data, advantages, values)
                    returns = [info['return'] for info in ep_infos]
                    logger.info('[TRPO] # %d: n_episodes = %d, returns: {mean = %.0f, std = %.0f}, '
                                'dist std = %.10f, dist mean = %.10f, vf_loss = %.3f',
                                n_updates, len(returns), np.mean(returns), np.std(returns) / np.sqrt(len(returns)),
