...
def main():
    ...
    train_sets = [Dataset(dtype, FLAGS.rollout.max_buf_size) for i in range(5)]
    ...
    if FLAGS.ckpt.buf_load:
        ...
        for i in range(FLAGS.ckpt.buf_load_index):
            ...
            if i <= 0:
                add_multi_step(data, train_sets[4])
            elif i <= 1:
                add_multi_step(data, train_sets[3])
            elif i <= 5:
                add_multi_step(data, train_sets[2])
            elif i <= 24:
                add_multi_step(data, train_sets[1])
            elif i <= 99:
                add_multi_step(data, train_sets[0])
    ...
    while TASK_NUM < FLAGS.task.n_iters:
        ...
        for i in range(FLAGS.warmup.n_iters):
            ...
            for _ in range(n_model_iters):
                samples = train_sets[0].sample_multi_step(FLAGS.model.train_batch_size, 1, FLAGS.model.multi_step)
            ...
            if i % FLAGS.model.validation_freq == 0:
                samples = train_sets[0].sample_multi_step(
            ...
            for n_updates in range(FLAGS.warmup.n_policy_iters):
                if FLAGS.algorithm != 'MF' and FLAGS.warmup.start == 'buffer':
                    runners['train'].set_state(train_sets[0].sample(FLAGS.plan.n_envs).state)
                else:
                    runners['train'].reset()
        ...
        for T in range(FLAGS.slbo.n_stages):
            ...
            for i in range(FLAGS.slbo.n_iters):
                ...
                for _ in range(n_model_iters):
                    samples = train_sets[max(T, 4)].sample_multi_step(FLAGS.model.train_batch_size, 1, FLAGS.model.multi_step)
                    _, train_loss, grad_norm = loss_mod.get_loss(
                        samples.state, samples.next_state, samples.action, ~samples.done & ~samples.timeout,
                        fetch='train loss grad_norm')
                    losses.append(train_loss.mean())
                ...
                if i % FLAGS.model.validation_freq == 0:
                    samples = train_sets[max(T, 4)].sample_multi_step(
                        FLAGS.model.train_batch_size, 1, FLAGS.model.multi_step)
                    loss = loss_mod.get_loss(
                        samples.state, samples.next_state, samples.action, ~samples.done & ~samples.timeout)
                    loss = loss.mean()
                ...
                for n_updates in range(FLAGS.slbo.n_policy_iters):
                    if FLAGS.algorithm != 'MF' and FLAGS.slbo.start == 'buffer':
                        runners['train'].set_state(train_sets[max(T, 4)].sample(FLAGS.plan.n_envs).state)
                    else:
                        runners['train'].reset()
